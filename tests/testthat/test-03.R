...
test_that("Testing simple reactions", {
  sys <- isopath() %>%
  ...
  expect_error(sys %>% add_simple_reaction(A == C, alpha.carbon.eq = 1), "equilibrium .* unclear")
  expect_error(sys %>% add_simple_reaction(A == C, alpha.carbon.eq = 1, eq_ratio = "err"), "equilibrium .* unclear")
  expect_is( sys %>% add_simple_reaction(A == C, alpha.carbon.eq = 1, eq_ratio = "P/S"), "isopath")
  ...
  expect_error(sys %>% add_simple_reaction(A == B, alpha.carbon = 1, alpha.nitrogen.eq = 1, eq_ratio = "P/S"),
               "ambiguous reaction type")
  expect_warning(sys %>% add_simple_reaction(A == B, alpha.carbon = 1, alpha.nitrogen = 1, eq_ratio = "P/S"),
                                             "defined but no equilibrium fractionation factor")
  expect_is( sys %>% add_simple_reaction(A == B, alpha.carbon.eq = 1, eps.nitrogen.eq = 1, eq_ratio = "P/S"), "isopath")
  ...
  expect_equal(sys %>% add_simple_reaction(A == C, alpha.carbon.eq = ceq, flux = my_flux, eq_ratio = "S/P") %>%
                 get_ode_matrix(),
               data_frame(
  ...
  expect_equal((sys %>% add_simple_reaction(A == C, alpha.carbon.eq = ceq, flux = my_flux, eq_ratio = "P/S") %>%
  ...
  expect_equal((sys %>% add_simple_reaction(A == C, alpha.carbon = cff.fwd, alpha.carbon.eq = cff.eq, eq_ratio = "S/P",
                                           flux = my_flux, reversibility = my_rev) %>%
                 get_flux_isotope_matrix())$flux_isotope[4],
               "fractionate(C.carbon, a = cff.fwd/cff.eq)")
  expect_equal((sys %>% add_simple_reaction(A == C, alpha.carbon = cff.fwd, alpha.carbon.eq = cff.eq, eq_ratio = "P/S",
                                            flux = my_flux, reversibility = my_rev) %>%
                  get_flux_isotope_matrix())$flux_isotope[4],
               "fractionate(C.carbon, a = cff.fwd * cff.eq)")
  expect_equal((sys %>% add_simple_reaction(A == C, alpha.carbon = cff.fwd, eps.carbon.eq = eff.eq, eq_ratio = "S/P",
                                            flux = my_flux, reversibility = my_rev) %>%
                  get_flux_isotope_matrix())$flux_isotope[4],
               "fractionate(C.carbon, a = cff.fwd/(0.001 * eff.eq + 1))")
  expect_equal((sys %>% add_simple_reaction(A == C, alpha.carbon = cff.fwd, eps.carbon.eq = eff.eq, eq_ratio = "P/S",
                                            flux = my_flux, reversibility = my_rev) %>%
                  get_flux_isotope_matrix())$flux_isotope[4],
               "fractionate(C.carbon, a = cff.fwd * (0.001 * eff.eq + 1))")
})
